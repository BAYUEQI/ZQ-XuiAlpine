...
config_after_install() {
    ...
    # 使用api64.ipify.org以同时支持IPv4和IPv6
    local server_ip=$(curl -s https://api64.ipify.org)
...
install_x-ui() {
	cd /usr/local
    if [ $# == 0 ]; then
        tag_version=$(curl -Ls "https://api.github.com/repos/BAYUEQI/ZQ-XuiAlpine/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$tag_version" ]]; then
            echo -e "${red}获取x-ui版本失败, 可能是GitHub API限制或网络连接失败, 请检查您的网络连接后重试...${plain}"
            exit 1
        fi
        echo -e "获取x-ui最新版本: ${tag_version}, 开始安装..."
        wget --no-check-certificate -O /usr/local/x-ui-linux-alpine.tar.gz https://github.com/BAYUEQI/ZQ-XuiAlpine/releases/download/${tag_version}/x-ui-linux-alpine.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        url="https://github.com/BAYUEQI/ZQ-XuiAlpine/releases/download/${tag_version}/x-ui-linux-alpine.tar.gz"
        echo -e "开始安装x-ui $1"
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://raw.githubusercontent.com/BAYUEQI/ZQ-XuiAlpine/master/x-ui-alpine.sh
    chmod +x /usr/bin/x-ui
    wget --no-check-certificate -O /etc/init.d/x-ui https://raw.githubusercontent.com/BAYUEQI/ZQ-XuiAlpine/master/x-ui.rc
