...
legacy_version() {
    ...
    install_command="bash <(curl -Ls \"https://raw.githubusercontent.com/BAYUEQI/ZQ-XuiAlpine/v$tag_version/install_alpine.sh\") v$tag_version"
...
uninstall() {
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/BAYUEQI/ZQ-XuiAlpine/master/install_alpine.sh)${plain}"
    echo ""
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://raw.githubusercontent.com/BAYUEQI/ZQ-XuiAlpine/master/x-ui-alpine.sh
    if [[ $? != 0 ]]; then
        echo ""
